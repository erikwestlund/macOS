...
youtube_app="$HOME/Applications/Chrome Apps.localized/YouTube.app"
chrome_bin="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
info_plist="$youtube_app/Contents/Info.plist"
...
previous_window_id=$(
  /opt/homebrew/bin/yabai -m query --windows | /opt/homebrew/bin/jq -r \
    ...
    '[.[] | select(.app == "YouTube" and .space == $current_space)] | sort_by(.id) | last | .id // 0'
)
...
for _ in {1..40}; do
  new_window_id=$(
    /opt/homebrew/bin/yabai -m query --windows | /opt/homebrew/bin/jq -r \
      ...
      '[.[] | select(.app == "YouTube" and .space == $current_space and .id > $previous_window_id)] | sort_by(.id) | last | .id // empty'
  )
